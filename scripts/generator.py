...
def build_prompt(keyword, lang, related_phrases=None, related_articles=None):
...
"""
    ...
    if related_articles:
        ...
        links_section = f"""
INTERNAL LINKS — these articles already exist on this site.
Where naturally and genuinely relevant in the text or tips, link to 1-3 of them
using markdown format [Title](/lang/slug). Only link where it genuinely helps
the reader — never force it. Do not link in every sentence.
{links_list}
"""
...
def generate_article(keyword, lang, related_phrases=None, related_articles=None):
    try:
        response = call_with_timeout(
            client.models.generate_content,
            90,  # сек — этот вызов сложнее и дольше, даём больше запаса
            model='gemini-2.5-flash',
            contents=build_prompt(keyword, lang, related_phrases, related_articles),
            config=types.GenerateContentConfig(
                temperature=0.7,
...
def load_articles_meta():
    """Загружает уже опубликованные статьи для внутренних ссылок."""
    meta_path = ROOT / 'data' / 'articles-meta.json'
...
def get_related_for_linking(keyword, lang, articles_meta, limit=4):
    """
    Возвращает до `limit` опубликованных статей того же языка как кандидаты
    для внутренних ссылок. Отбираем статьи, у которых нет значимого пересечения
    слов с текущим ключевиком (не хотим ссылаться на дубли) — берём разные темы.
    """
    articles = articles_meta.get(lang, [])
    if not articles:
        return []

    stop = {'how','to','a','an','the','what','when','where','why','do','can',
            'i','you','it','is','are','and','or','be','in','on','at','for',
            'with','from','of','my','your',
            'wie','was','wo','wann','warum','hoe','wat','wanneer','hur','man'}
    ...
    for a in articles:
        if not a.get('slug') or not a.get('title'):
            continue
        a_sig = sig(a.get('keyword', '') + ' ' + a.get('title', ''))
        # пропускаем статьи со слишком большим пересечением (вероятные дубли)
        overlap = len(kw_sig & a_sig) / max(len(kw_sig | a_sig), 1)
        if overlap < 0.5:
            candidates.append((overlap, a))

    # Берём разнообразные статьи: сортируем по убыванию пересечения
    # (чуть похожие темы — лучше чем совсем несвязанные, ссылка выглядит
    # естественнее), но исключаем слишком похожие (overlap ≥ 0.5)
...
def run_generator(lang=None, limit=None):
    ...
    # Загружаем мета опубликованных статей — нужны для внутренних ссылок
    ...
    for keyword, kw_lang, searches, cpc, related_json in keywords:
        ...
        # Подбираем кандидатов для внутренних ссылок
        related_articles = get_related_for_linking(keyword, kw_lang, articles_meta)
        if related_articles:
            print(f'   🔗 {len(related_articles)} статей предложены для внутренних ссылок')

        article = generate_article(keyword, kw_lang, related_phrases, related_articles)
        if article:
